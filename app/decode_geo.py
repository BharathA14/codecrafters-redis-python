...
def compact_int64_to_int32(v: int) -> int:
    """
    Compacts a 64-bit integer back to a 32-bit integer by removing
    the interleaved zero bits.
    """
...
def decode(geo_code: int) -> tuple[float, float]:
    """
    Decode a geohash score back to latitude and longitude.

    Args:
        geo_code: 64-bit integer geohash score

    Returns:
        tuple of (latitude, longitude)
    """
    # Step 1: Separate the interleaved bits
    y = geo_code >> 1  # Extract longitude bits
    x = geo_code  # Extract latitude bits

    # Step 2: Compact back to 32-bit integers
    grid_latitude_number = compact_int64_to_int32(x)
    grid_longitude_number = compact_int64_to_int32(y)

    # Step 3: Convert back to geographic coordinates with higher precision
    # Use more precise arithmetic to reduce rounding errors
    scale_factor = float(2**26)

    grid_latitude_min = MIN_LATITUDE + LATITUDE_RANGE * (
        grid_latitude_number / scale_factor
    )
    grid_latitude_max = MIN_LATITUDE + LATITUDE_RANGE * (
        (grid_latitude_number + 1) / scale_factor
    )
    grid_longitude_min = MIN_LONGITUDE + LONGITUDE_RANGE * (
        grid_longitude_number / scale_factor
    )
    grid_longitude_max = MIN_LONGITUDE + LONGITUDE_RANGE * (
        (grid_longitude_number + 1) / scale_factor
    )

    latitude = (grid_latitude_min + grid_latitude_max) * 0.5
    longitude = (grid_longitude_min + grid_longitude_max) * 0.5
...
if __name__ == "__main__":
    ...
    for test_case in test_cases:
        ...
        print(f"{test_case['name']}: (lat={decoded_latitude},lon={decoded_longitude}) {status}")
        if not success:
